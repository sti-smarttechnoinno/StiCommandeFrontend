...
import { X, MapPin, Phone, Mail, Users, ShoppingCart, DollarSign, TrendingUp, Activity, BarChart3, Pencil, FileText, Eye } from 'lucide-react';
...
export function WilayaDrawer({ wilayaId, onClose }: WilayaDrawerProps) {
  const wilaya = mockWilayas.find((w) => w.id === wilayaId);
              ...
              <CardContent className="p-4">
                <div className="flex items-end gap-1 h-20">
                  {wilaya.revenueTrend.map((val, i) => {
                    const max = Math.max(...wilaya.revenueTrend);
