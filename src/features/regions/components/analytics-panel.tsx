...
const REGIONAL_REVENUE_DATA = [
  { name: 'Algiers / Center', value: 85000000, color: '#2563EB' },
  { name: 'Oran / West', value: 52000000, color: '#22C55E' },
  { name: 'Constantine / East', value: 41000000, color: '#8B5CF6' },
  { name: 'Ouargla / South', value: 20500000, color: '#F59E0B' },
];

const TOP_REGIONAL_DELEGATES = [
  { name: 'Yacine B.', region: 'Center', orders: 156, revenue: 12500000, completion: 96 },
  { name: 'Amine K.', region: 'West', orders: 142, revenue: 11200000, completion: 94 },
  { name: 'Sofiane M.', region: 'East', orders: 128, revenue: 9800000, completion: 91 },
  { name: 'Rachid T.', region: 'South', orders: 115, revenue: 8500000, completion: 89 },
];

const WILAYA_STATUS_SUMMARY = [
  { label: 'Active Coverage', count: 48, color: 'bg-emerald-500', textColor: 'text-emerald-600 dark:text-emerald-400', bgColor: 'bg-emerald-500/10 border-emerald-500/20' },
  { label: 'Limited Operations', count: 8, color: 'bg-amber-500', textColor: 'text-amber-600 dark:text-amber-400', bgColor: 'bg-amber-500/10 border-amber-500/20' },
  { label: 'Pending Expansion', count: 2, color: 'bg-blue-500', textColor: 'text-blue-600 dark:text-blue-400', bgColor: 'bg-blue-500/10 border-blue-500/20' },
...
function DonutChart() {
  const total = REGIONAL_REVENUE_DATA.reduce((s, r) => s + r.value, 0);
    ...
    <div className="flex items-center gap-4">
      <div className="relative w-28 h-28 flex-shrink-0">
        <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
          {REGIONAL_REVENUE_DATA.map((segment) => {
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-1">
          <span className="text-[11px] font-bold text-foreground leading-tight">198,500,000</span>
          <span className="text-[9px] text-muted-foreground">DA Total</span>
        </div>
      </div>
      <div className="flex-1 space-y-1.5">
        {REGIONAL_REVENUE_DATA.map((region) => (
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-stretch w-full">
      ...
      <Card className="h-full border border-border/40 shadow-xs rounded-2xl overflow-hidden flex flex-col justify-between">
        ...
        <CardContent className="p-4 flex-1 space-y-2.5">
          {TOP_REGIONAL_DELEGATES.map((delegate, i) => (
      ...
      <Card className="h-full border border-border/40 shadow-xs rounded-2xl overflow-hidden flex flex-col justify-between">
        <CardHeader className="flex flex-row items-center justify-between gap-4 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription className="text-xs text-muted-foreground mt-0.5">
                58 Wilayas operational breakdown
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-4 flex-1 flex flex-col justify-between">
          <div className="grid grid-cols-2 gap-3.5 h-full">
            {WILAYA_STATUS_SUMMARY.map((item) => {
              const pct = Math.round((item.count / 58) * 100);
              return (
                <div
                  key={item.label}
                  className={cn(
                ...
                >
                  ...
                  <div className="mt-3 flex items-baseline justify-between">
                    <span className={cn('text-3xl font-extrabold tracking-tight', item.textColor)}>
                      {item.count}
                    </span>
